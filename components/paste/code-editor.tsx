...
export function CodeEditor({ value, onChange, language, maxLength = 500000 }: CodeEditorProps) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setHighlightedHtml(`<pre><code>${value.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</code></pre>`)
      } finally {
  ...
  return (
    <div className="flex flex-col rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] overflow-hidden">
